...
const loadReminders = async () => {
  ...
  try {
    ...
    // 个人提醒只关注当前用户，不需要传入familyId
    // 如果传入familyId，后端会切换到家庭看板模式
...
const generateSmartReminders = async () => {
  loading.value = true
  try {
    const res = await generateSmartRemindersApi()
    if (res && res.code === 0 && res.data) {
      const count = res.data.length || 0
      if (count > 0) {
        ElMessage.success(`成功生成 ${count} 条智能提醒`)
      } else {
